...
public class Algoritm {
    ...
    public void getHAsleshB() {
        for (int j=0; j<lengthJ; j++) {
            hAsleshB += bj[j] * hASleshBj[j];
        }
        System.out.println("Загальна умовна ентропія джерела А відносно джерела В = " + hAsleshB);
    }

    public void getHBsleshA() {
        for (int i=0; i<heightI; i++) {
            hBsleshA += ai[i] * hBSleshAi[i];
        }
        System.out.println("Загальна умовна ентропія джерела B відносно джерела A = " + hBsleshA);
    }

    public void getAllEntropia() {
        System.out.println("Ентропія об'єднання цих джерел = " + (hB+hAsleshB) +
                "\n" + "Кількість інформації, що припадає на одне повідомлення = " + (hA-hAsleshB));
    }
}
